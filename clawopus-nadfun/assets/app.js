/* ===========================================================
   CLAWOPUS · nad.fun (Monad) — front-end logic
...
(function () {
  ...
  const bootLines = [
    ...
    "[ ok ] connecting to Monad RPC ............ online",
    "[ ok ] linking nad.fun deployer ........... ready",
  ...
  async function fillStats() {
    // Try live data from the local server (which proxies the nad.fun indexer).
    let s = null;
    try {
    ...
    const data = live
      ? {
          launched: Number(s.launched) || 0,
          today: Number(s.today) || 0,
          deployed: null, // not exposed by the public indexer
          score: null,
        }
      : { launched: 42, today: 6, deployed: 318.7, score: 0.74 };

    const set = (name, fn) => {
      const el = document.querySelector(`[data-stat="${name}"]`);
      if (el) fn(el);
    };
    set("launched", (el) => animateValue(el, data.launched));
    ...
    set("pnl", (el) => {
      ...
      else { el.textContent = "+128.4 " + CONFIG.currency; el.classList.add("up"); }
    });

    const noteEl = document.querySelector("[data-scope-note]");
    if (noteEl) {
      if (live && s.scope === "network") {
        noteEl.textContent = "↑ live network-wide nad.fun totals (not agent-only). set CLAWOPUS_CREATOR to track a specific wallet. deployed / score / P&L aren't exposed by the public API.";
      } else if (live && s.scope === "agent") {
        noteEl.textContent = "↑ live tokens created by the agent wallet " + (s.creator || "");
      } else {
        noteEl.textContent = "↑ demo data — run `npm run web` for live nad.fun stats.";
      }
    }
  ...
  const TICKERS = ["MONKE", "PURPL", "GMON", "NADCAT", "OCTO", "WAGMON", "DEGEN", "PEPEMON", "BLOB", "CLAW"];
  const NARR = [
    "monad mainnet szn narrative spiking",
    "ct calling a purple summer",
    "new nad.fun volume ATH detected",
  ...
  function feedLine() {
    ...
    if (roll < 0.34) {
      ...
      return t + `<span class="ok">launch</span> deployed <span class="acc">$${rand(TICKERS)}</span> on ${CONFIG.platform} · ${(2 + Math.random() * 8).toFixed(1)} ${CONFIG.currency} seeded · score ${score}`;
    } else {
  ...
  function runFeed(state) {
    ...
    if (live) {
      appendFeed(log, `<span class="t">[${clock()}]</span> <span class="muted">watching nad.fun for new launches…</span>`);
      // seed (oldest first so newest ends at the bottom)
      (state.recent || []).slice().reverse().forEach((t) => pushToken(t, "new"));
      // poll the indexer for genuinely new launches
      async function poll() {
        try {
          const res = await fetch("/api/state", { cache: "no-store" });
          if (!res.ok) return;
      ...
      // demo / file:// mode — clearly simulated
      appendFeed(log, `<span class="t">[${clock()}]</span> <span class="warn">demo feed — run \`npm run web\` for real nad.fun launches</span>`);
  ...
  const HANDLERS = {
    ...
    async status() {
      termPrint("querying nad.fun indexer ...", "muted");
      let s = null;
      try {
        const res = await fetch("/api/state", { cache: "no-store" });
        if (res.ok) s = await res.json();
      ...
      if (!s) {
        ...
        s = { live: false, level: 7, launched: 42, today: 6, scope: "mock" };
      }
      const na = s.live ? "n/a (not in public API)" : "—";
      termPrint(
        "  source        " + (s.live ? "LIVE · " + (s.scope || "network") : "demo / mock") + "\n" +
        "  status        " + (s.status || (s.live ? "online" : "offline")) + "\n" +
        "  launched      " + (s.launched ?? 0) + (s.scope === "network" ? "  (network-wide)" : "") + "\n" +
        "  today         " + (s.today ?? 0) + "\n" +
    ...
    async launch(args) {
      ...
      await termPrintSlow([
        ...
        { text: "  ↳ seeding liquidity " + (CONFIG.currency === "BNB" ? (0.2 + Math.random() * 1.4).toFixed(2) : (2 + Math.random() * 8).toFixed(1)) + " " + CONFIG.currency, cls: "muted" },
      ], 420);
